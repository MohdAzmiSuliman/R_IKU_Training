...
tibble(Package = c("tidyverse", "haven", "readxl", "lubridate", 
                   "ggplot2", "dplyr", "stringr", "forcats"), 
...
asthma_ds0 <- read_sav("Dataset/asthma_ds.sav")
```
...
first_half <- tidyverse_table[1:5, ]
second_half <- tidyverse_table[6:10, ]
```
...
kable(first_half) %>% kable_classic()
```
...
kable(second_half) %>% kable_classic()
```
...
```{r}
asthma_ds5 <- read_sav("Dataset/asthma_ds.sav") %>%
  as_factor() %>%
